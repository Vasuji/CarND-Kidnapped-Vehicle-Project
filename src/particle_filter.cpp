...
// reference: https://github.com/zlatanajanovic/CarND-Kidnapped-Vehicle-Project/blob/master/src/particle_filter.cpp
...
void ParticleFilter::init(double x, double y, double theta, double std[]) {
	...
	for (int i = 0; i < num_particles; ++i) {
        ...
        // collect all particles into particles vector
		particles.push_back(p);
		
		 // collect all weights into particles vector
		weights.push_back(p.weight);

		// cout << "Sample " << i + 1 << ": " << p.x << endl;
	}
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
	...
	for (int i=0; i<num_particles; ++i) {
			...
			// this will be filled with transformed landmarks
			...
			// Predict map_landmarks (a map object) in particle's frame (includes rotation and translation)
			for (int j = 0; j < map_landmarks.landmark_list.size(); ++j) {
					...
					if (dist(x, y, landmark_x, landmark_y) < sensor_range) {
							...
							transformed_landmark.x = cos(-theta) * (landmark_x - x) - sin(-theta) * (landmark_y - y);
							transformed_landmark.y = sin(-theta) * (landmark_x - x) + cos(-theta) * (landmark_y - y);
							transformed_landmark.id = map_landmarks.landmark_list[j].id_i;
							predicted.push_back(transformed_landmark);
			...
			// check how close we are for predicted and observation using nearest neighbor technique
			...
			for (int j = 0; j < observations.size(); ++j) {
                    ...
                    // each measurements are independent. Multiply all individual likelyhood
                ...
                // calculate total sum of weight for normalization
